...
$SimsTutorialBtn = $window.FindName('SimsTutorialBtn')
...
$RunSimsDlcBtn          = $window.FindName('RunSimsDlcBtn')
 # New Sims page buttons for installation/update and unlocker download
 $SimsInstallBtn        = $window.FindName('SimsInstallBtn')
 $SimsUnlockerBtn       = $window.FindName('SimsUnlockerBtn')
...
if ($SimsTutorialBtn) {
    $null = $SimsTutorialBtn.Add_Click({
        try {
            # Βάλε εδώ το δικό σου tutorial link
            Start-Process 'https://www.youtube.com/watch?v=dQw4w9WgXcQ'
        } catch {
            [System.Windows.MessageBox]::Show("Δεν ήταν δυνατή η εκκίνηση του tutorial.", "Sims Tools")
        }
    })
}

if ($SimsInstallBtn) {
    $null = $SimsInstallBtn.Add_Click({
        param($src, $evt)
        try {
            # Create working directory in temp
            $workDir = Join-Path $env:TEMP 'SimsTools'
            if (-not (Test-Path $workDir)) {
                New-Item -ItemType Directory -Force -Path $workDir | Out-Null
            }
            # Download Sims install/update package
            $installerUrl = 'https://www.dropbox.com/scl/fi/5841qp2eysq0xvsxodmr1/sims_install.zip?rlkey=h843bkdkw8ymi7rqaq473ktni&st=1e4980yk&dl=1'
            $zipPath = Join-Path $workDir 'sims_install.zip'
            Invoke-FileDownload -Uri $installerUrl -Destination $zipPath | Out-Null
            Expand-Archive -Path $zipPath -DestinationPath $workDir -Force -ErrorAction Stop
            Remove-Item -Path $zipPath -Force -ErrorAction SilentlyContinue
            $entry = Get-ChildItem -Path $workDir -Recurse -File | Where-Object {
                $_.Extension -in '.ps1', '.bat', '.cmd', '.exe'
            } | Select-Object -First 1
            if ($entry) {
                if ($entry.Extension -eq '.ps1') {
                    Start-Process -FilePath 'powershell.exe' -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$($entry.FullName)`"" -Verb RunAs
                } elseif ($entry.Extension -eq '.bat' -or $entry.Extension -eq '.cmd') {
                    Start-Process -FilePath $entry.FullName -Verb RunAs
                } elseif ($entry.Extension -eq '.exe') {
                    Start-Process -FilePath $entry.FullName -Verb RunAs
                }
            } else {
                [System.Windows.MessageBox]::Show("Κατέβηκε το πακέτο εγκατάστασης Sims αλλά δεν βρέθηκε εκτελέσιμο αρχείο.", "Sims Tools")
            }
        } catch {
            [System.Windows.MessageBox]::Show("Σφάλμα κατά την εγκατάσταση/ενημέρωση: $($_.Exception.Message)", "Sims Tools", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Error) | Out-Null
        }
    })
}

if ($SimsUnlockerBtn) {
    $null = $SimsUnlockerBtn.Add_Click({
        param($src, $evt)
        try {
            # Create working directory in temp
            $workDir = Join-Path $env:TEMP 'SimsTools'
            ...
            # Download EA DLC Unlocker package
            $unlockerUrl = 'https://www.dropbox.com/scl/fi/mbnhjivbbyyn4avjerrzw/EA_UNLOCKER.zip?rlkey=vchua3ks8whlvmgbvc2kas0mw&st=fypc2au2&dl=1'
            $zipPath = Join-Path $workDir 'ea_unlocker.zip'
            Invoke-FileDownload -Uri $unlockerUrl -Destination $zipPath | Out-Null
            Expand-Archive -Path $zipPath -DestinationPath $workDir -Force -ErrorAction Stop
            Remove-Item -Path $zipPath -Force -ErrorAction SilentlyContinue
            $entry = Get-ChildItem -Path $workDir -Recurse -File | Where-Object {
                $_.Extension -in '.ps1', '.bat', '.cmd', '.exe'
            ...
            if ($entry) {
                ...
                [System.Windows.MessageBox]::Show("Κατέβηκε το πακέτο EA DLC Unlocker αλλά δεν βρέθηκε εκτελέσιμο αρχείο.", "Sims Tools")
            }
        } catch {
            [System.Windows.MessageBox]::Show("Σφάλμα κατά τη λήψη ή εκτέλεση του Unlocker: $($_.Exception.Message)", "Sims Tools", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Error) | Out-Null
        }
    })
}
...
# MIIcEgYJKoZIhvcNAQcCoIIcAzCCG/8CAQExDzANBglghkgBZQMEAgEFADB5Bgor
# BgEEAYI3AgEEoGswaTA0BgorBgEEAYI3AgEeMCYCAwEAAAQQH8w7YFlLCE63JNLG
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDGDIfk7LdJmNH2
# GH8UB3cJqpNHF3ykKQdH1r6RtIQcc6CCFl4wggMgMIICCKADAgECAhAt0Itv5Cbq
# v0e3x0R8xr1KMA0GCSqGSIb3DQEBCwUAMBoxGDAWBgNVBAMMD0tvbG9raXRoZXMg
# QS5FLjAeFw0yNTA5MDMwMjA5NTBaFw0yNjA5MDMwMjE5NTBaMBoxGDAWBgNVBAMM
# D0tvbG9raXRoZXMgQS5FLjCCASIwDQYJKoZIhvcNAQEBBQADggEPADCCAQoCggEB
# AMcNaENC/YgmLvw1ZvIwpZ6JiGrU0/cASHm0qKG0A4J2hjS/4tArh2PGJCbLP3zI
# WivkRyTVHgG5fGsT53Yfefor+OFXuVt07qZnbAOb/s0RFcdwZ+Xsv/HuL2G/VK8Z
# NMnqtAI5qMRazMkMpPP8UKgZMrvny73/hFYNqFi7hOzSzPGlNhX8uYqTCgX5v5ky
# DfraG6T0koxf3o8gOJHB3L9+/LggRL9ZdUBAaskOBkyckJEiZuVobBdJdb8OyT7w
# 60t/CZNcmv7qxL83Nuu1N6Tf7vr2PlCoLTKUc6DCVHChzkSvX7mV5D6U5r4bwXjN
# tdwoyqHS8E5xF533w4SGH/ECAwEAAaNiMGAwDgYDVR0PAQH/BAQDAgeAMBMGA1Ud
# JQQMMAoGCCsGAQUFBwMDMBoGA1UdEQQTMBGCD0tvbG9raXRoZXMgQS5FLjAdBgNV
# HQ4EFgQUTHC72iw/uwUoYUodxsClhYM721kwDQYJKoZIhvcNAQELBQADggEBAARG
# n+1gcDruv+EoyG4SP2pohEZKlAi+AVUgpquDEleuOGmRLvaVsEmSM0QjPWfcgg6T
# 4Kg2g6eNe9UuY/nX4M9FqUx+QgPHmW9jQF6Z2239fitRtXRjW/awnXP4E7iTopAq
# eTgEG7yWrfu//Xp1RkHyxnSO3nYzaCyibVxbNx/FtMPThp/kjh78cdQC31juwQCw
# eq47VqZiWakXLTuCM+cAascChG/lvGoeUDvooAOQkumVoUT2PbA9x9b641DVUlCT
# A8pe7AcP794lZ4ZL4b4kl2pT2cTBKbO6aY3hwxp3Hi1GqnwX6BvktDXasnES/Hlm
# y8UK+I+M9v3HxjDHbhEwggWNMIIEdaADAgECAhAOmxiO+dAt5+/bUOIIQBhaMA0G
